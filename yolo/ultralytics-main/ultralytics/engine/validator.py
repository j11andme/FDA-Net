        ...
        for batch_i, batch in enumerate(bar):
            ...
            with dt[2]:
                if self.training:
                    ...
                    # 如果当前 loss 只有 3 项 (box, cls, dfl)，但 self.loss 有 5 项
                    if len(loss_items) < len(self.loss):
                        diff = len(self.loss) - len(loss_items)
                        # 在后面补 0 (fsa=0, aug=0)，因为验证集不计算这两个 Loss
                        loss_items = torch.cat([loss_items, torch.zeros(diff, device=loss_items.device)])
