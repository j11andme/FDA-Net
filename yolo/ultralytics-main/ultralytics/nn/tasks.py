...
class DuDetectionModel(BaseModel):
    """
    YOLO detection model.
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        detect_module.register_forward_hook(self._align_hook)

    def _align_hook(self, mod, inp, out):
            feats = inp[0]         
            self.align_feats = feats
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is C3k2:  # for M/L/X sizes
                ...
                if scale in "mlx":
                    args[3] = True
            if m is SAE_YOLO:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            if m is A2C2f:
